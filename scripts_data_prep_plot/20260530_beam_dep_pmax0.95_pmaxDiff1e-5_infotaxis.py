from pathlib import Path

from simulation_data_prep import (
    load_summary, 
    load_ph,
    gather_success_info, 
    get_success_fname,
)

# Stopping criteria
search_type = "infotaxis"
pmax_th = 0.95
pmax_diff_th = 1e-5
simulation_type = "beam_dep"  # "simple", "mismatch", or "beam_dep"


# Path to save output csv
path_main = Path("/Volumes/ssd_2tb_1/infotaxis_simu_2026")
path_summary_ph = path_main / "20260522_summary"
path_csv = path_main / "20260530_infotaxis_beam_dep_summary_compiled_pmax095_pmaxDiff1e-5"
if not path_csv.exists():
    path_csv.mkdir(parents=True, exist_ok=True)


# Parameters to loop through
cr_all = [5, 10]
br_all = [1, 2]
pm_all = [0.001, 0.002, 0.003, 0.004, 0.005, 0.01]

beam_sigma_r = {
    1: 0.5,
    2: 1,
}
beam_scale_all = [3, 5]

for cr in cr_all:
    for br in br_all:
        for beam_scale in beam_scale_all:
            for pm in pm_all:
                print("------------------------------------------------")
                print(f"cr={cr}, br={br}, pm=pfa={pm}, beam_sigma_r={beam_sigma_r[br]}, beam_scale={beam_scale}")

                # Load summary and ph details
                df_y = load_summary(
                    search_type=search_type,
                    cr=cr, br=br,
                    path_data=path_summary_ph,
                    simulation_type=simulation_type,
                    pm_agent=pm, pm_truth=pm,
                    beam_sigma_r=beam_sigma_r[br],
                    beam_scale=beam_scale
                )

                # Get num_pings when p_max crosses threshold
                ds_y = load_ph(
                    search_type=search_type, 
                    cr=cr, br=br, 
                    path_data=path_summary_ph,
                    simulation_type=simulation_type,
                    pm_agent=pm, pm_truth=pm,
                    beam_sigma_r=beam_sigma_r[br],
                    beam_scale=beam_scale
                )

                # Make df_y index 1-based to match with ds_y "run" dimension
                df_y.index = df_y.index + 1

                # Gather success info based on pmax_th and pmax_diff_th
                df_y = gather_success_info(
                    df_y=df_y,
                    ds_y=ds_y,
                    pmax_th=pmax_th,
                    pmax_diff_th=pmax_diff_th
                )

                # Determine output filename
                csv_fname = get_success_fname(
                    search_type=search_type,
                    cr=cr, br=br,
                    pm_agent=pm, pm_truth=pm,
                    simulation_type=simulation_type,
                    pmax_th=pmax_th, pmax_diff_th=pmax_diff_th,
                    beam_sigma_r=beam_sigma_r[br], beam_scale=beam_scale
                )
                df_y.to_csv(path_csv / csv_fname)

                print("save refined info to:")
                print(f" - {csv_fname}")
