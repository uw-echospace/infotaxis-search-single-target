...
# def beam_dep_curve(r, dip, r_ref=None):
#     """
#     Producing a pm/pfa curve with a depression in the middle )r=0
#     The curve is horizontally scaled to match either r_max or largest of the input r.max()
#     The curve is generated using an inverted cosine with the negative peak scaled
#     :param r: distance of current point(s)
#     :param dip: height of the middle depression of pm/pfa
#     :param r_ref: reference distance
#     :return:
#     """
#     if r_ref is None:
#         if r.size == 1:  # if there is only 1 element
#             r_ref = 1    # set reference distance = 1 (say within the beam)
#         else:                # if there are multiple elements
#             r_ref = r.max()  # set to the largest distance
#     return 1-(1-dip)*(np.cos(r / r_ref * np.pi) / 2 + 0.5)
#
#
#
#
# def pmpfa_fcn_old(fcn_type, k_cell, k_axis, simu_param,
#               k_target=None, dist_opt=True, verbose_opt=False):
#     """Generate beam- and/or location-dependent probability of false alarm.
#
#     Parameters
#     ----------
#     fcn_type : str
...
#         flat index of beam axis
#     simu_param : dict
#         simu_param['d_shp']           search space dimension
#         simu_param['pm_const']        pm/pfa as a constant when not beam-dependent; default=0.01
#         simu_param['pfa_const']
#         simu_param['pm_mtx']          pm/pfa as a function of search space; default=np.nan when not location-dependent
#         simu_param['pfa_mtx']
#         simu_param['pm_beam_fcn']     optional; name of the function that generates the beam-dependent curve
#         simu_param['pfa_beam_fcn']
#         simu_param['pm_target_fcn']   optional; name of the function that generates the target-dependent curve
#         simu_param['pfa_target_fcn']
#     k_target : int, optional
#         flat index of target, if not supplied, assume uniform
#     dist_opt : bool
#         if to overwrite content of param_animal and
#         turn distance-dependent (wrt beam/target) functions off
#         set to True when using pfa_fcn in cases in which k_cell not in the beam
#         such as in calculating p_X1_Tk_B0 under update_X1
#     verbose_opt : bool
#         if to print detailed messages
#
#     Returns
...
#     """
#
#     # Get search space dimension
#     try:
#         d_shp = simu_param['d_shp']
#     except KeyError:
#         print('Matrix shape not supplied in simu_param, exiting function...')
#         return
#
#     # load background pm or pfa
#     try:
...
#         p_const = 0.001
#
#     # if location-dependent
#     try:
#         p_mtx_k_cell = simu_param['%s_mtx' % fcn_type].ravel()[k_cell]
#         if verbose_opt:
#             print('%s_mtx supplied, %s is location dependent.' % (fcn_type, fcn_type))
#     except KeyError:
#         p_mtx_k_cell = np.ones((d_shp[0]*d_shp[1],))[k_cell] * p_const  # default is uniform with pm_const
#         if verbose_opt:
#             print('No %s_mtx supplied, %s is not location dependent.' % (fcn_type, fcn_type))
#
#     # get ij indices
#     ij_cell = np.array(np.unravel_index(k_cell, d_shp))  # force to be np array
#     ij_axis = np.array(np.unravel_index(k_axis, d_shp))
#
#     # if beam-dependent
#     if dist_opt:  # if care about distance from beam axis to specific cell
#         try:
#             r_beam = cdist(np.array([ij_axis, ] * int(ij_cell.size / 2)).reshape(-1, 2),
#                            ij_cell.T.reshape((-1, 2)), 'euclidean')[0, :]
#             p_beam = simu_param['%s_beam_fcn' % fcn_type][0](r_beam, *simu_param['%s_beam_fcn' % fcn_type][1])
#             if verbose_opt:
#                 print('Beam-dependent %s_mtx supplied, %s is beam dependent.' % (fcn_type, fcn_type))
#         except KeyError:
#             p_beam = np.ones((k_cell.size,))
#             if verbose_opt:
#                 print('No beam-dependent %s_mtx supplied, %s is not beam dependent.' % (fcn_type, fcn_type))
#     else:
#         p_beam = np.ones((k_cell.size,))
#         if verbose_opt:
#             print('No beam-dependent %s_mtx supplied, %s is not beam dependent.' % (fcn_type, fcn_type))
...
#     if dist_opt:  # if care about distance from beam axis to specific cell
#         try:
#             ij_target = np.array(np.unravel_index(k_target, d_shp))
#             r_target = cdist(np.array([ij_target, ] * int(ij_cell.size / 2)).reshape(-1, 2),
#                              ij_cell.T.reshape((-1, 2)), 'euclidean')[0, :]
#             p_target = simu_param['%s_target_fcn' % fcn_type][0](r_target, *simu_param['%s_target_fcn' % fcn_type][1])
#         except TypeError:  # is k_target is None
#             if verbose_opt:
#                 print('Warning: Need to supply k_target in simu_param if want to enable pfa_target. ' +
#                       'Use default uniform for now.')
#             p_target = np.ones((k_cell.size,))
#         except KeyError:
#             p_target = np.ones((k_cell.size,))
#             if verbose_opt:
#                 print('Warning: Need to supply p_target_fcn if want to enable pfa_target. Use default uniform for now.')
#     else:
#         p_target = np.ones((k_cell.size,))
#
#     # location-dependent pfa applied as a multiplier
...
#
#
# def update_X0_old(d, k_axis, k_in_beam, simu_param, verbose_opt=False):
#     """Update dk when NO echo was received (X=0).
#
#     Parameters
#     ----------
#     d
#         probability of target at each cell within the search space
#     k_axis : int
#         flat index of beam axis
#     k_in_beam : array
#         flat indices of cells covered by beam
#     simu_param : dict
#         modeling params re pm/pfa functions (animal's expectation)
#     verbose_opt : bool
#         whether to print out detailed messages
#
#     Returns
...
#
#     # Get flattened indices
#     k_not_in_beam = np.setdiff1d(np.arange(d.size), k_in_beam, assume_unique=True)  # cells not covered by beam
#
#     # Sanity check
#     if np.sum(dk_new[k_in_beam]) == 1:  # if dk in all cells are 1
#         if verbose_opt:
#             print('Probability of all cells in beam sums to 1. No updates performed.')
#         return d
#     else:
#         # For cells covered by the beam
#         for kk in k_in_beam:
#             pm_kk = pmpfa_fcn(fcn_type='pm', k_cell=kk, k_axis=k_axis, simu_param=simu_param)
#             k_in_beam_not_kk = np.setdiff1d(k_in_beam, kk, assume_unique=True)
#             p_X0_Tk_Bk = pm_kk * np.prod(1-pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam_not_kk,
#                                                      k_axis=k_axis, simu_param=simu_param))
#             dk_new[kk] = p_X0_Tk_Bk * dk[kk]
#
#         # For cells not covered by the beam
#         for kk_n in k_not_in_beam:
#             p_X0_Tk_B0 = np.prod(1 - pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam, k_axis=k_axis,
#                                                dist_opt=False, simu_param=simu_param))
#             dk_new[kk_n] = p_X0_Tk_B0 * dk[kk_n]
#
#         # Normalize and reshape to get dk map
#         # The if statement is to avoid the case when all cells outside the beam are 0
#         # This case is problematic in the updating only when PM is 0, since in all other
...
#
#
# def update_X1_old(d, k_axis, k_in_beam, simu_param, verbose_opt=False):
#     """Update dk when an echo was received (X=1).
#
#     Parameters
#     ----------
...
#
#     # Update dk
#     if np.all(dk_new[k_in_beam] == 0):  # if dk in all cells are 0
#         if verbose_opt:
#             print('All cells in beam has probability 0. No updates performed.')
#         return d
#     else:
#         # For cells covered by the beam
#         for kk in k_in_beam:  # given target at kk
#             pm_kk = pmpfa_fcn(fcn_type='pm', k_cell=kk, k_axis=k_axis, simu_param=simu_param)
#             k_in_beam_not_kk = np.setdiff1d(k_in_beam, kk, assume_unique=True)
#             p_X1_Tk_Bk = 1 - pm_kk * np.prod(1-pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam_not_kk,
#                                                          k_axis=k_axis, simu_param=simu_param))
#             dk_new[kk] = p_X1_Tk_Bk * dk[kk]
#
#         # For cells not covered by the beam
#         for kk_n in k_not_in_beam:  # given target at kk_n
#             p_X1_Tk_B0 = 1 - np.prod(1 - pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam, k_axis=k_axis,
#                                                    dist_opt=False, simu_param=simu_param))
#             dk_new[kk_n] = p_X1_Tk_B0 * dk[kk_n]
#
#         # Normalize and reshape to get dk map
#         dk_new = dk_new / np.sum(dk_new)
#         return np.reshape(dk_new, d.shape)
#
#
# def get_h_est_old(dd, simu_param, beam_cover_dim=None):
#     """Loop through all possible axis location to calculate the expected entropy.
#
#     Parameters
#     ----------
#     dd : ndarray
...
#         modeling params re pm/pfa functions (animal's expectation)
#     beam_cover_dim : int
#         dimension of beam size
#         default None: 3x3 beam
#         beam_cover_dim=5: 5x5 beam
#
#     Returns
#     -------
#         h_next, h_X0_est, h_X1_est, p_X0_est, p_X1_est
#     """
...
#     h_next = np.empty(d_curr.shape)  # mtx to store estimated entropy
#     h_X0_est = np.empty(d_curr.shape)  # mtx to store estimated entropy if next X=0
#     h_X1_est = np.empty(d_curr.shape)  # mtx to store estimated entropy if next X=1
#     p_X0_est = np.empty(d_curr.shape)  # mtx to store estimated next p(X=0)
#     p_X1_est = np.empty(d_curr.shape)  # mtx to store estimated next p(X=1)
#
#     for b in np.array(b_ij).T:
#         # print(b[0]*b[1])
#         # if b[0]*b[1] == 225:
#         #     print(b)
#         # for (pingnum,b) in zip(np.arange(d_curr.size),np.array(b_ij).T):
#         # New beam cover
#         cover_ij = b + beam_shape(beam_cover_dim)
#         cover_ij = clean_beam_cover(cover_ij, d_curr)
#
#         # Get flattened indices
#         k_axis = np.ravel_multi_index((b[0], b[1]), simu_param['d_shp'])    # beam axis
#         k_in_beam = np.ravel_multi_index((cover_ij[:, 0], cover_ij[:, 1]), simu_param['d_shp'])  # cells covered by beam
#
#         # Estimate d and h
#         #     print('beam axis num  %d' % pingnum)
#         d_est_X1 = update_X1(dk=d_curr, k_axis=k_axis, k_in_beam=k_in_beam,  # next d mtx when received echo
#                              simu_param=simu_param)
#         d_est_X0 = update_X0(dk=d_curr, k_axis=k_axis, k_in_beam=k_in_beam,  # next d mtx when no echo
#                              simu_param=simu_param)
#         d_est_X1 = d_est_X1.ravel()  # flatten the d mtx
#         d_est_X0 = d_est_X0.ravel()
#         h_X1 = -np.sum(xlogy(d_est_X1, d_est_X1))  # avoid 0*log0 error
#         h_X0 = -np.sum(xlogy(d_est_X0, d_est_X0))
#         h_X1 = loge_to_log2(h_X1)  # convert to base 2
#         h_X0 = loge_to_log2(h_X0)
...
#         # Probability of receiving an echo or not
#         # Get flattened indices
#         k_in_beam = np.ravel_multi_index((cover_ij[:, 0], cover_ij[:, 1]), d_curr.shape)  # cells in beam
#         k_b = np.ravel_multi_index(b, d_curr.shape)   # linear index for current beam axis
#         p_tB = np.sum(dk_curr[k_in_beam])  # probability of target in beam
#
#         # Probability of receiving an echo: p_X1
#         # --- p(X=1 | target in B)
#         p_X1_tB_tmp = np.zeros((k_in_beam.size, 1))
#         for kk, kseq in zip(k_in_beam, np.arange(k_in_beam.size)):
#             # print(kseq)
#             k_in_beam_not_kk = np.setdiff1d(k_in_beam, kk, assume_unique=True)
#             if dk_curr[kk] == 0:
#                 p_X1_tB_tmp[kseq] = 0
#             else:
#                 p_X1_tB_tmp[kseq] = (1 -
#                                      pmpfa_fcn(fcn_type='pm', k_cell=kk, k_axis=k_b, simu_param=simu_param) *
#                                      np.prod(1 - pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam_not_kk, k_axis=k_b,
#                                                            simu_param=simu_param))) * \
#                                     (dk_curr[kk] / np.sum(dk_curr[k_in_beam]))
#         p_X1_tB = np.sum(p_X1_tB_tmp)
#         # --- p(X=1 | target not in B)
#         p_X1_tnB = 1 - np.prod(1 - pmpfa_fcn(fcn_type='pfa', k_cell=k_in_beam, # dist_opt=True: beam is in effect
#                                              k_axis=k_b, simu_param=simu_param))
#         # --- Put everything together
#         p_X1 = p_X1_tB * p_tB + p_X1_tnB * (1-p_tB)
#
#         # expected entropy
#         h_next[b[0], b[1]] = p_X1 * h_X1 + (1 - p_X1) * h_X0
#         h_X0_est[b[0], b[1]] = h_X0
#         h_X1_est[b[0], b[1]] = h_X1
#         p_X0_est[b[0], b[1]] = 1 - p_X1
#         p_X1_est[b[0], b[1]] = p_X1
#
#     return h_next, h_X0_est, h_X1_est, p_X0_est, p_X1_est
#
