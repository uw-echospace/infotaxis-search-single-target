...
parser.add_argument(
    ...
    type=int,
    default=500,
    help="maximum number of pings allowed in the search"
)
...
max_ping_num = args.max_ping_num
...
for idx, row in df_param.iterrows():
    ...
    pm_agent = float(row["pm_agent"])
    pfa_agent = float(row["pfa_agent"])
    pm_truth = float(row["pm_truth"])
    pfa_truth = float(row["pfa_truth"])
    canvas_radius = int(row["canvas_radius"])

    # beam-dependent pm/pfa params
    beam_sigma_r = float(row["beam_sigma_r"])
    beam_scale = float(row["beam_scale"])
    ...
    simu_params = {
        ...
        "param_animal": {  # agent internal model for pm/pfa
            str(beam_radius): {  # pm/pfa const indexed by beam_radius
                "pm_const": pm_agent,
                "pfa_const": pfa_agent,
                "pm_beam_sigma_r": beam_sigma_r,
                "pfa_beam_sigma_r": beam_sigma_r,
                "pm_beam_scale": beam_scale,
                "pfa_beam_scale": beam_scale,
            },
        },
        "param_echo": {  # true pm/pfa in search space
            str(beam_radius): {  # pm/pfa const indexed by beam_radius
                "pm_const": pm_truth,
                "pfa_const": pfa_truth,
                "pm_beam_sigma_r": beam_sigma_r,
    ...
    param_echo_str = []
    for k,v in simu_params["param_animal"].items():  # agent model params
        param_echo_str.append(
            f"_beamRadius{k}_pmAgent{v["pm_const"]:.0e}_pfaAgent{v["pfa_const"]:.0e}"
            f"_beamSigmaR{v["pm_beam_sigma_r"]:.0e}_beamScale{v["pm_beam_scale"]:.0e}"
        )
    for k,v in simu_params["param_echo"].items():  # search space truth params
        param_echo_str.append(
            f"_beamRadius{k}_pmTruth{v["pm_const"]:.0e}_pfaTruth{v["pfa_const"]:.0e}"
            f"_beamSigmaR{v["pm_beam_sigma_r"]:.0e}_beamScale{v["pm_beam_scale"]:.0e}"
        )
    path_save = (
    ...
    oth = one_target.OneTargetHex(
        ...
        param_animal= simu_params["param_animal"],
    ...
    # Load back nc file to get max dk cube
    ds = xr.open_dataset(path_save / f"{fname_prefix}.nc", engine="netcdf4")
    max_dk_cube = ds["canvas_cube"].values[ds["d_all"].argmax(dim="canvas").values]
    target_cube = ds.attrs["target_loc"]
    ds = xr.Dataset(
        {
            "max_dk_cube": (("ping", "cube_dim",), max_dk_cube),
            "target_cube": (("cube_dim",), target_cube),
        },
        coords={
            "cube_dim": [1, 2, 3],
            "ping": range(max_dk_cube.shape[0]),
        },
    )
    ds.to_netcdf(path_save / f"{fname_prefix}_dk_max_cube.nc")
